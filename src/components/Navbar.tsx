...
import { ShoppingBag, Menu, X, LogOut, History, ShieldCheck, LayoutDashboard, Search } from 'lucide-react';
...
export const Navbar = () => {
  ...
  return (
    <nav className={cn(
      ...
      scrolled 
        ? "bg-white/30 dark:bg-black/30 backdrop-blur-3xl border-b border-white/10 py-2 shadow-2xl" 
        : "bg-white/5 dark:bg-black/5 backdrop-blur-sm py-4"
    )}>
      <div className="container mx-auto">
        <div className="h-14 md:h-16 flex items-center justify-between gap-4">
          <Link href="/">
            <Logo variant={scrolled ? 'dark' : 'light'} size="md" className="shrink-0" />
          </Link>
          ...
          <div className="flex flex-col p-6 gap-2">
            <form onSubmit={handleSearchSubmit} className="mb-4 relative">
              ...
              <Input 
                ...
                className="h-12 pl-12 rounded-2xl bg-secondary/50 border-none"
              />
            </form>
            ...
            {!user ? (
              ...
              <button 
                ...
                className="px-6 py-4 font-black uppercase tracking-widest text-[11px] text-destructive text-left hover:bg-destructive/5 rounded-2xl mt-4"
              >
                Sign Out
              </button>
            )}
